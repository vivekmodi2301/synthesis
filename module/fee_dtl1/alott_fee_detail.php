...
?>
    <div class="col-lg-12 table-responsive" style="margin-left:10px; font-family:'Century Gothic';">
                ...
                <?php 
                    $sno=1;
                    $class_fee_data=fetchAll("select class.name as class, batch.name as batch,class_fee_dtl.id,reg_fee,tution_fee,test_series_fees,study_matirial_fee,gst_per,gst_value,total_fee from class_fee_dtl join class on class_id=class.id join batch on batch_id=batch.id");
                    foreach ($class_fee_data as $class_fee_value) {
                ?>
                        <tr class="primary1 table_style2">
                            <td><?php echo $sno++; ?></td>
                            <td><?php echo $class_fee_value['batch'] ?></td>
