<?php
    ...
    if(!isset($_POST['where'])){
        ...
        if($no_of_cchq!=''){
            if(!$no_having)
                $having.=" having sum(statuss='cleared')=$no_of_cchq";
            else
                $having.=" and sum(statuss='cleared')=$no_of_cchq";
            $no_having = 1;
            $_SESSION['having']=$having;
        }
        if($no_of_bchq!=''){
            if(!$no_having)
                $having.=" having sum(statuss='bounced')=$no_of_bchq";
            else
                $having.=" and sum(statuss='bounced')=$no_of_bchq";
            $_SESSION['having']=$having;
            $no_having = 1;
        ...
        if($no_of_uchq!=''){
            if(!$no_having)
                $having.=" having sum(update_date!='0000-00-00')=$no_of_uchq";
            else
                $having.=" and sum(update_date!='0000-00-00')=$no_of_uchq";
            $_SESSION['having']=$having;
            $no_having = 1;
        ...
        if($no_of_echq!=''){
            ...
            if(!$no_having)
                ...
                $having.=" having sum(expiry_date>$curr_date)=$no_of_echq";
            else
                $having.=" and sum(expiry_date>$curr_date)=$no_of_uchq";
            $_SESSION['having']=$having;
            $no_having = 1;
        }
    }else{
...
<tr>
    <td colspan="14"><a href="<?php echo ROOT ?>module/pdc_chq/search_print_see_pchq.php?wh=<?php echo $wh; ?>&having=<?php echo $having; ?>" class="btn btn-primary">Print</a></td>
</tr>
<tr class="active1 table_style" style="font-weight:600;">
    ...
    <?php
        ...
        // print_r($_POST);
        @PaginationWork(10); 
        $totRslt['tot']=mysqli_num_rows(mysqli_query($con,"select count(*) as tot from stu_profile left join pdc on pdc.roll=stu_profile.roll_no $wh and wheree='addmission'  group by roll_no $having"));
        // echo "select count(*) as tot from stu_profile left join pdc on pdc.roll=stu_profile.roll_no $wh and wheree='addmission'  group by roll_no $having";
        ?>
        <script type="text/javascript">
            $('#tot_stu').html('<?php echo $totRslt['tot']; ?>')
        </script>
        <?php  
    $qry="select roll_no as roll,sum(pdc.amount) as amount,count(amount) as given,stu_profile.name,f_name,scholarship,class_id,batch_id,statuss,old_student from stu_profile left join pdc on pdc.roll=stu_profile.roll_no $wh and wheree='addmission'  group by roll_no $having limit ".$frmdata['from'].", ".$frmdata['to'];
    // echo "select roll_no as roll,sum(pdc.amount) as amount,count(amount) as given,stu_profile.name,f_name,scholarship,class_id,batch_id,statuss from stu_profile left join pdc on pdc.roll=stu_profile.roll_no $wh and wheree='addmission'  group by roll_no $having limit ".$frmdata['from'].", ".$frmdata['to'];
