...
elseif($a_from=='' && $a_to!=''){
    ...
    $pdc_dtl=fetchAll("select pdc.id,pdc.amount,roll,pdc.chq_no,bank_name,pdc.datee,expiry_date,stu_profile.name,f_mobile,update_date from pdc join stu_profile on pdc.roll=stu_profile.roll_no where (datee<'$current_date'  and statuss='recived') and refund='n' $wh");
   // echo "<tr><td>select pdc.id,pdc.amount,roll,pdc.chq_no,bank_name,pdc.datee,expiry_date,stu_profile.name,f_mobile,update_date from pdc join stu_profile on pdc.roll=stu_profile.roll_no where (datee>'$current_date'  and statuss='recived') $wh</td></tr>";
