...
?>
    ...
    <table class="table table-bordered table-hover" id="show">
        <tr>
            <td colspan="14"><a href="<?php echo ROOT ?>module/pdc_chq/search_print_see_pchq.php" class="btn btn-primary">Print</a></td>
        </tr>
        <tr class="active1 table_style" style="font-weight:600;">
        ...
        $url=ROOT."index/pdc_chq/syn_s_chq/";
            ...
            $frmdataget=$_REQUEST;
            @PaginationWork(10); 
            $totRslt['tot']=mysqli_num_rows(mysqli_query($con,"select count(*) as tot from stu_profile left join pdc on roll=roll_no where wheree='addmission' GROUP by roll_no"));
            ?>
        ...
        $qry="select roll_no as roll,f_name,name,sum(amount),count(amount),scholarship,class_id,batch_id,statuss,old_student from stu_profile left join pdc on roll=roll_no where wheree='addmission' GROUP by roll_no limit ".$frmdata['from'].", ".$frmdata['to'];
