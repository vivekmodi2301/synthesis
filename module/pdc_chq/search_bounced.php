...
$week_complete_date=date('Y-m-d',$next_timestamp);
    $pdc_dtl=fetchAll("select pdc.id,pdc.amount,roll,pdc.chq_no,bank_name,pdc.datee,expiry_date,stu_profile.name,f_mobile,update_date from pdc join stu_profile on pdc.roll=stu_profile.roll_no where (datee>='$current_date' and datee<='$week_complete_date' and statuss='bounced') $wh");
  // echo "<tr><td>select pdc.id,pdc.amount,roll,pdc.chq_no,bank_name,pdc.datee,expiry_date,stu_profile.name,f_mobile from pdc join stu_profile on pdc.roll=stu_profile.roll_no where datee>='$current_date' and datee<='$week_complete_date' and statuss='bounced' $wh</td></tr>";
