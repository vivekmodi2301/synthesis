	...
	<table class="table table-bordered table-hover">
        ...
        <table class="table table-bordered table-hover">
            ...
            <tr class="active1 table_style" style="font-weight:600; text-align:justify;">
                    <td colspan="6" align="center">You are here at : <span style="font-size:16px; color:#900;">Upcoming P.D.C. List</span></td>
                </tr>
            	<tr class="active1 table_style" style="font-weight:600; text-align:justify;">
                    <td>Total Students in this Section : <span id="tot_stu" class="profile_chng_color"></span></td>
                    <td>Upcoming P.D.C.</td>
                   <?php /*?> <td>Send Msg<br><input type="checkbox" id="send_msg"></td>
                    <td>Move To <select onchange="changeStatus(this.value,'recived')" class="form-control" id="class" name="class">
                        <option value="">Select</option>
                        <option value="cleared">Cleared Cheque Section</option>
                        <option value="bounced">Bounced Cheque Section</option>
                        <!-- <option value="">Updated Cheque Section</option> -->
                    </td><?php */?>
                    <td><button name="" id="" class="btn btn-success btn-sm btn-block" style="font-family:'Century Gothic'; font-size:14px; font-weight:bold;" onclick="printPage('upcoming','')">SEND REMINDER MSG</button></td>
            ...
            <table class="table table-bordered table-hover" id="show">
                ...
                <?php 
                    ...
                    $qry="select pdc.id,pdc.amount,roll,pdc.chq_no,bank_name,pdc.datee,expiry_date,stu_profile.name,f_mobile,update_date from pdc join stu_profile on pdc.roll=stu_profile.roll_no where ((datee>='$current_date' and datee<='$week_complete_date' and statuss='recived') or (update_date>='$current_date' and update_date<='$week_complete_date' and statuss='recived')) and refund='n' ";
                   // echo $qry;
            ...
            if(moveTo==''){
                if(yourArray.length>0){
                    ...
                    $.ajax({
                        ...
                        data:"number="+yourArray+"&statuss="+statuss,
                        type:"post",
                        success:function(e){
...
<script type="text/javascript">
      function search(){
        ...
        var a_to=$('#a_to').val();
        $.ajax({
            url:root_path+"module/pdc_chq/search_upcoming.php",
