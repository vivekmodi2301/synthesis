    ...
    <table class="table table-bordered table-hover">
                <tr class="active1 table_style" style="font-weight:600; text-align:justify;">
                    <td colspan="4" align="center">You are at : <span style="font-size:16px; color:#900;">Cleared P.D.C. List</span></td>
                </tr>
                
                <tr class="active1 table_style" style="font-weight:600; text-align:justify;">
                    <td>Total Students in this Section : <span id="tot_stu" class="profile_chng_color"></span></td>
                    <td>Cleared P.D.C.</td>
                    <td>Move To <select onchange="changeStatus(this.value,'cleared')" class="form-control" id="class" name="class">
                        <option value="">Select</option>
                        
                        <option value="bounced">Bounced Cheque Section</option>
                        <!-- <option value="">Updated Cheque Section</option> -->
                        <!-- <option value="expired">Expired Cheque Section</option> -->
                        <option value="recived">Recived Cheque Section</option>
                    </td>
                    <td><button name="" id="" class="btn btn-success btn-sm btn-block" style="font-family:'Century Gothic'; font-size:14px; font-weight:bold;" onclick="printPage('cleared','')">SEND MSG</button></td>                    
            ...
            <table class="table table-bordered table-hover" id="show">
                ...
                <?php 
                    ...
                    $qry="select pdc.id,pdc.amount,roll,pdc.chq_no,bank_name,pdc.datee,expiry_date,stu_profile.name,f_mobile from pdc join stu_profile on pdc.roll=stu_profile.roll_no where  statuss='cleared'";
                    // echo $qry;
    ...
    <script type="text/javascript">
      function search(){
        ...
        var a_to=$('#a_to').val();
        $.ajax({
            url:root_path+"module/pdc_chq/search_cleared.php",
