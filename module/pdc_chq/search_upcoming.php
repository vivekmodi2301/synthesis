...
if($d_from!='' && $d_to==''){
    $wh.=" and expiry_date='$d_from'";
}
elseif($d_from=='' && $d_to!=''){
    $wh.=" and expiry_date='$i_to'";
}elseif($d_from!='' && $d_to!=''){
    $wh.=" and (expiry_date>='$d_from' and expiry_date<='$d_to')";
}
if($a_from!='' && $a_to==''){
    $wh.=" and update_date='$a_from'";
}
elseif($a_from=='' && $a_to!=''){
    $wh.=" and update_date='$a_to'";
}elseif($a_from!='' && $a_to!=''){
    $wh.=" and (update_date>='$a_from' and update_date<='$a_to')";
} 
    ...
    $pdc_dtl=fetchAll("select pdc.id,pdc.amount,roll,pdc.chq_no,bank_name,pdc.datee,expiry_date,stu_profile.name,f_mobile,update_date from pdc join stu_profile on pdc.roll=stu_profile.roll_no where (((datee>='$current_date' and datee<='$week_complete_date' and statuss='recived') or (update_date>='$current_date' and update_date<='$week_complete_date' and statuss='recived') and statuss='recived')) and refund='n' $wh");
   // echo "<tr><td>select pdc.id,pdc.amount,roll,pdc.chq_no,bank_name,pdc.datee,expiry_date,stu_profile.name,f_mobile from pdc join stu_profile on pdc.roll=stu_profile.roll_no where ((datee>='$current_date' and datee<='$week_complete_date' and statuss='recived') or (update_date>='$current_date' and update_date<='$week_complete_date' and statuss='recived') and statuss='recived') $wh</td></tr>";
