<?php 
	if(isset($_GET['id'])){
		$id=$_GET['id'];
		$pay_data = fetchOne("select * from payment join expenses on bill_no = expenses.id  where payment.id=$id");
		// print_r($pay_data);
	}
	if(isset($_POST['amount'])){
		$_POST['pay_done'] = $_SESSION['logindtl']['id'];
		addUpdate('payment',$_POST,$id);
		?>
...
<form method="post">
	<table class="table table-bordered">
		...
		<tr>
			...
			<td><?php
			$org_name = fetchOne("select name from organization join expenses  on org_name = organization.id where expenses.id=$pay_data[bill_no]");
			 echo $org_name['name']; ?></td>
		</tr>
		<tr>
			<td>Purpose</td>
		...
		<tr>
			<td>Total Amount</td>
			<td><input type="text"  class="form-control" value="<?php echo $pay_data['total_amount']; ?>" id="tot"></td>
		</tr>
		<tr>
			<td>Paid</td>
			<td>
				<?php 
				$total_pay = fetchOne("select sum(amount) as tot from payment where bill_no=$pay_data[bill_no]");
				?>
				<input type="text" disabled  class="form-control" value="<?php echo $total_pay['tot']; ?>" id="pay"></td>
		</tr>
		<tr>
			<td>Amount</td>
			<td><input type="text" name="amount"  class="form-control" onchange="chk_amount(this.value,'<?php echo $pay_data['amount']; ?>')" id="now" value="<?php echo $pay_data['amount']; ?>"></td>
		</tr>
		<tr>
			<td>Date</td>
			<td><input type="date" name="datee" value="<?php echo $pay_data['datee']; ?>" class="form-control"></td>
		</tr>
		<tr>
			<td>Payment By</td>
			<td>
...
<script type="text/javascript">
	function chk_amount(now_pay,old_pay){
		// alert(now_pay);
		var tot = parseInt($('#tot').val());
		var pay = parseInt($('#pay').val());
		if(old_pay){
			old_pay = parseInt(old_pay);
			pay = pay-old_pay; 
		}
		// alert(pay);
